...
const LatestBlogPreview = () => {
  ...
  return (
    <section className="latest-blog-preview py-6 px-4 bg-white text-black">
      <div className="container mx-auto max-w-3xl">
        <h2 className="text-xl md:text-2xl font-heading mb-2 border-b border-gray-200 pb-2">Latest From Our Blog</h2>
        <div className="my-3">
          <h3 className="text-lg md:text-xl font-semibold">{latestPost.title}</h3>
          <p className="text-sm text-gray-500 mb-2">{new Date(latestPost.publishedAt).toLocaleDateString()}</p>
          <p className="text-base mb-3">
            {latestPost.excerpt || 'Check out our latest blog post!'}
          </p>
